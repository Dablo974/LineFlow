...
import { Pause, Play, X, Timer, Hourglass, ChevronLeft, ChevronRight, Bell, Home, History, ArrowLeft, Wand2, LoaderCircle, Sparkles, Images, Trash2, PenTool } from 'lucide-react';
...
const GeneratePoseInputSchema = z.object({
  description: z.string().min(10, "Please enter a more descriptive prompt to get better results.").describe('A text description of the character or pose to generate.'),
});
...
export default function AIPosesPracticePage() {
  ...
  const form = useForm<z.infer<typeof GeneratePoseInputSchema>>({
  ...
  const handleGenerateImages = async (values: GeneratePoseInput) => {
    ...
    try {
      ...
      toast({ title: "Images generated!", description: "Press Start Session to begin." });
        ...
        toast({
            ...
            description: "Could not generate images. The model may have refused the prompt. Please try again.",
            variant: "destructive"
        });
        setSessionState('idle');
    }
  ...
  const nextImageSrc = images[(currentImageIndex + 1) % images.length];
            ...
            <div className="p-4 space-y-6">
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleGenerateImages)} className="space-y-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2 text-lg"><PenTool className="size-5 text-primary" /> AI Pose Generator</CardTitle>
                      <CardDescription>Describe a character or pose, and we'll generate reference images.</CardDescription>
                    </CardHeader>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="e.g., A graceful dancer leaping through the air" {...field} disabled={isGenerating} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                ...
                <div className="text-center text-muted-foreground max-w-sm flex flex-col items-center gap-4">
                    {sessionState === 'generating' ? (
                       <div className="w-full max-w-md space-y-4">
                        ...
                        <>
                            <PenTool className="mx-auto h-16 w-16 mb-4 text-primary" />
                            <h2 className="text-3xl font-bold text-foreground">AI Pose Practice</h2>
                            <p className="mt-2 leading-relaxed">Describe a character or pose, set your session preferences, and hit "Generate" to start.</p>
                        </>
                    )}
                </div>
            )}
