...
const ANATOMY_PARTS = [
  'hands', 'feet', 'face', 'arms', 'legs', 'upper body front', 'upper body back', 'lower body front', 'lower body back'
];

const getAnatomyPartForDay = () => {
    const now = new Date();
...
export default function AnatomyChallengePage() {
  ...
  useEffect(() => {
    setPartOfDay(getAnatomyPartForDay());
  }, []);
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      ...
      const imagePromises = Array.from({ length: imageCount }, (_, i) => async () => {
        const result = await generatePose({ description: `A clean reference image of human ${partOfDay} for an artist to study.` });
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="relative">
                      <Button onClick={handleGenerateImages} className="w-full" disabled={isGenerating || !partOfDay}>
                        ...
                        <div className="space-y-2">
                          {images.map((imgSrc, index) => (
                            <div key={`${imgSrc.slice(-20)}-${index}`} className="flex items-center gap-2 p-1 rounded-md animate-in fade-in">
                              ...
                              <span className="text-sm truncate flex-1 capitalize">{partOfDay} #{index + 1}</span>
